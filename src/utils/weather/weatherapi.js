import { truncate3 } from "../truncate3";
...
export class WeatherApi {
    ...
    static isNightTime(data, epoch) {
        ...
        const sunrise = WeatherApi.parseAstroTime(forecastDay?.astro?.sunrise);
        const sunset = WeatherApi.parseAstroTime(forecastDay?.astro?.sunset);
    ...
    static getWeatherIcon(code, isNight = false) {
    ...
    static formatWind(windKph) {
        if (windKph === undefined || windKph === null) return "--";
        return truncate3(windKph);
    }

    static getWeatherSummary(data) {
        const current = data?.current ?? {};
        const localTimeEpoch = data?.location?.localtime_epoch ?? data?.current?.last_updated_epoch;
        const isNight = WeatherApi.isNightTime(data, localTimeEpoch);

        const hourly = WeatherApi.getHourlyForecast(data).map((hour) => ({
            ...hour,
            hourLabel: new Date(hour.time_epoch * 1000).toLocaleString("en-US", {
                hour: "numeric",
                hour12: true,
            }).toLowerCase(),
            iconType: WeatherApi.getWeatherIcon(hour.condition.code, WeatherApi.isNightTime(data, hour.time_epoch)),
            tempC: Math.round(hour.temp_c),
            feelsLikeC: Math.round(hour.feelslike_c),
        }));

        return {
            locationName: data?.location?.name || "Weather",
            conditionText: current?.condition?.text || "Loading weather",
            tempC: Math.round(current?.temp_c ?? 0),
            feelsLikeC: Math.round(current?.feelslike_c ?? 0),
            humidity: current?.humidity ?? 0,
            windDirection: current?.wind_dir || "--",
            windKph: current?.wind_kph,
            iconType: WeatherApi.getWeatherIcon(current?.condition?.code, isNight),
            isNight,
            hourly,
        };
    }

    static WeatherIcon = (props) => {
        const icons = {
